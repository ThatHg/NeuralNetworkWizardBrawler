...
public:
	...
	double		GrabReal(const char* name)		const;
	const char*	GrabString(const char* name)	const;
	bool		GrabBoolean(const char* name)	const;
	int			GrabInteger(const char* name)	const;
...
inline bool Config::GrabBoolean(const char* name) const
{
	...
	const bool boolean = (bool)lua_toboolean(m_state, -1);
	lua_pop(m_state, 1);
	return boolean;
}
